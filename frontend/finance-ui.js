...
<p class="owner-note">Comissões são pagas na aba Equipe ou Caixa, nunca como gasto. Retirada do dono reduz o caixa, mas não o lucro.</p>`);
...
renderFinanceiro=async function() {
  ...
  try {
    ...
    const [p,extrato]=await Promise.all([ownerJson('/financeiro/posicao'),ownerJson('/financeiro/extrato?'+query)]);
    ...
    el.innerHTML=`${warning}<div class="kpi-grid fin-cash-kpis">${finValue(p.saldo_inicial_configurado?'Saldo atual':'Saldo registrado',p.saldo,'Entradas recebidas − saídas pagas')}${finValue('Valores reservados',p.comissoes_pendentes+p.contas_pendentes,'Comissões e contas ainda a pagar')}${finValue('Saldo livre',p.saldo_livre,'Saldo atual − todas as obrigações pendentes',true)}</div>
      <div class="fin-toolbar">${finButton('aporte','Registrar aporte')}${finButton('retirada','Registrar retirada')}${finButton('nova-conta','Nova despesa ou conta')}</div>
